...
public interface VentanillaRepository extends JpaRepository <Ventanilla, Long> {
    ...
    @Query(value = "SELECT v.id_ventanilla, v.estatus, v.nombre_ventanilla FROM ventanillas v INNER JOIN ventanilla_has_empleado vhe ON v.id_ventanilla = vhe.id_ventanilla WHERE vhe.id_ventanilla IS NOT NULL AND vhe.estatus = 1", nativeQuery = true)
